...
//Особеннось фабричного метода позволяет добавить еще и другие виды транспорта,
//при этом не нужно будет переписывать весь код, а достаточно будет добавить новый класс и его фабрику, которые наследуют соответствующие классы
...
//Создаем класс общей "руководящей" фабрики, которая имеет виртуальный метод create,
//переопределяемый для каждой подфабрики
class Factory{
public:
...
//Теперь можно создать функцию сборки, которая будет принимать фабрику и
//собирать нам транспорт, не зависимо от его типа
